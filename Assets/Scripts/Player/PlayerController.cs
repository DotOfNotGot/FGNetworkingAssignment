...
public class PlayerController : NetworkBehaviour, IPlayerActions
{
    ...
    public NetworkVariable<float> currentMovementSpeed = new NetworkVariable<float>();
    
    public override void OnNetworkSpawn()
    {
        currentMovementSpeed.Value = movementSpeed;
        if (!IsOwner) return;
    ...
    {
        if (!IsOwner) return;
        _rb.velocity = transform.up * (_moveInput.y * currentMovementSpeed.Value);
    ...
    public void SetMovementSpeed(float newMovementSpeed, float durationInSeconds)
    {
        StartCoroutine(SetMovementSpeedTimer(newMovementSpeed, durationInSeconds));
    }
    
    private IEnumerator SetMovementSpeedTimer(float newMovementSpeed, float durationInSeconds)
    {
        currentMovementSpeed.Value = newMovementSpeed;
